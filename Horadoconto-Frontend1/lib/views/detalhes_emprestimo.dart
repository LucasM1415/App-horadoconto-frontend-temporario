...
Future<void> criarEmprestimo(String usuarioId, String livroId,
  ...
  var url =
      'http://192.168.0.107:8080/emprestimo/criarEmprestimo'; // Substitua pela URL da sua API
...
class _DetalhesEmprestimoState extends State<DetalhesEmprestimo> {
  ...
  Future<void> fetchLivro() async {
    final response = await http.get(
        Uri.parse('http://192.168.0.107:8080/livros/buscar/${widget.livroId}'));
    if (response.statusCode == 200) {
            ...
            children: <Widget>[
              if (livro != null) ...[
                Expanded(
                  flex: 1,
                  child: Image.network(
                    'http://192.168.0.107:8080${livro!.imagem_capa}', // Use a propriedade 'imagem_capa' do seu modelo 'Livro'
